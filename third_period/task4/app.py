...
BASIC_NAMES = ["create", "is_empty", "insert", "is_element", "count", "search", "delete", "delete_all"]
CHALLENGE_NAMES = ["to_list", "union"]
...
.task4-point-score { background:rgba(11,11,13,.06); color:rgba(11,11,13,.78); }
...
def replace_title(html):
    ...
    return html.replace(
        ...
        '<span class="period-main">3期</span><span class="period-week">課題4</span></span>',
    )
...
def _is_unimplemented_challenge_question(question_summary):
    """Return True when to_list/union is absent and should not be shown."""
...
def _add_points(article, score, total):
    ...
    replacement = ("<div class='task4-status-row'>{}"
                   "<span class='task4-point-score'>{}点/{}点</span></div>").format(
                       marker, score, total)
...
def build_result_html(all_results, file_summaries):
    ...
    for summary in file_summaries:
        ...
        implemented_label = {
            frozenset(["to_list"]): "to_listのみ実装",
            frozenset(["union"]): "unionのみ実装",
            frozenset(["to_list", "union"]): "to_list / union 実装済み",
    ...
    for basic, challenge, summary in zip(basic_articles, challenge_articles, basic_summaries):
        score = sum(q.get("status") == "OK" for q in summary["questions"]) * 2
        decorated = _add_points(basic, score, 16)
